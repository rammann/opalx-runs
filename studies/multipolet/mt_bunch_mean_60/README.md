...
**MULTIPOLET** sector bend, **60 deg**, tanh fringe 0.02 m. Gaussian-bunch case for test 10 -- centroid shift from mean energy. No space charge; linear.
...
The covariance transforms as Sigma_out = M Sigma_in M^T with the map M measured in `mt_fringe_60`. The bunch mean momentum is P0 (1 + <delta>) while the field reference stays P0, so the bunch is uniformly off-energy and its centroid disperses: <x>_out = D <delta> (test 10). OPALX's reference particle rides with its own bunch, so the shift is read from the reference orbit against the on-energy `mt_bunch_emit_60` orbit.

## Setup (`mt_bunch_mean_60.in`)
