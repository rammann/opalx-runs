...
The design particle rides an arc of radius rho = L/theta = 1.9099 m turning through theta = 30 deg. Rigidity Brho = 0.3353 T m, so the dipole field is `TP[0] = Brho/rho = -0.1755 T`, `TP[1] = +0.5 T/m`. A magnetic field does no work, so |p| is conserved (test 1).
...
- `Case.read_plane` reads the co-moving phase space from `mt_cf_30.h5` (positions [m] relative to the reference particle, momenta in beta*gamma rotated into the reference direction): x' = px/pz, y' = py/pz, delta = |p|/p0 - 1.
- `Case.transfer_map` takes a field-free plane on each side of the bend (field extent from `By_ref` in `mt_cf_30.stat`), projects every particle onto the reference transverse plane, strips the drift to the design face, and finite-differences the +/- pairs.
