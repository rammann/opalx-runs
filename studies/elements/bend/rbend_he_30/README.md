...
**RBEND** bend at **30°**, hard edge (HGAP=0). 0.1 GeV electron, no space charge.
...
- Lattice: `DRIFT(L=1) → RBEND(L=1, ANGLE=30°, HGAP=0) → DRIFT`. Design faces at s = 1.000 .. 2.012 m. Dumps are dense (`PSDUMPFREQ` small) so read planes land near the faces.

## How the output is processed (`bendlib.py` → `run_tests.py`)

- `Case.read_plane` reads the co-moving phase space from `rbend_he_30.h5`: positions are in the beam frame [m], momenta in β·γ rotated into the reference direction → x′ = px/pz, y′ = py/pz, δ = |p|/p₀ − 1.
- `Case.transfer_map` builds the 6×6 map: pick a field-free plane on each side of the bend (the field extent comes from `By_ref` in `rbend_he_30.stat`), project every particle onto the reference transverse plane (drift by −z), strip the exact drift from each plane to the design face, then centered-finite-difference the ± pairs.
