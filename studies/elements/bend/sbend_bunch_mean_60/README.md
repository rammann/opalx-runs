...
**SBEND** bend at **60°**, Enge fringe. Gaussian-bunch case for **test 10 -- centroid shift from mean energy**. No space charge; linear, no radiation.
...
- OPALX's reference particle rides *with* its own (off-energy) bunch, so the shift shows up in the **reference orbit**, not in mean_x. `centroid_orbit_shift` compares this case's reference orbit to the on-design `sbend_bunch_emit_60` orbit and projects the separation onto the design perpendicular.
