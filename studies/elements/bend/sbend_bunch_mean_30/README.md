...
**SBEND** bend at **30°**, Enge fringe. Gaussian-bunch case for **test 10 -- centroid shift from mean energy**. No space charge; linear, no radiation.
...
The bunch covariance transforms as **Σ_out = M·Σ_in·Mᵀ** with the same map M measured in the map case `sbend_enge_30`. This case is the special projection:
...
- Same lattice as `sbend_enge_30`: `DRIFT → SBEND 30° (HGAP=0.01) → DRIFT`.

## How the output is processed (`bendlib.py` → `run_tests.py`)

- OPALX's reference particle rides *with* its own (off-energy) bunch, so the shift shows up in the **reference orbit**, not in mean_x. `centroid_orbit_shift` compares this case's reference orbit to the on-design `sbend_bunch_emit_30` orbit and projects the separation onto the design perpendicular.
