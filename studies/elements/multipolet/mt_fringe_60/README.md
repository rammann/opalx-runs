...
- `Case.read_plane` reads the co-moving phase space from `mt_fringe_60.h5` (positions [m] relative to the reference particle, momenta in beta*gamma rotated into the reference direction): x' = px/pz, y' = py/pz, delta = |p|/p0 - 1.
- `Case.transfer_map` takes a field-free plane on each side of the bend (field extent from `By_ref` in `mt_fringe_60.stat`), projects every particle onto the reference transverse plane, strips the drift to the design face, and finite-differences the +/- pairs.
