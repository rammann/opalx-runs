...
The same quadrupole field written the awkward ways real files are: a comment block in front, rows out of order, nine columns.
...
## Setup (`grid_quad_messy.in`)

- Map `../maps/quad_messy.g4blmap`, grid format, carried by a `FIELDMAP`.
...
Must reproduce grid_quad to round-off, which is what says the comment block, the row order and the zero electric field columns make no difference.
