...
grid_sol again with twice the time step, to show where the residual against the closed form comes from.
...
- 0.1 GeV electron, no space charge, time step 2e-13 s, tracking stops at
  2.0 m.
...
The residual against the closed form should be about twice grid_sol's, which is what says it is the step resolving the two hard map ends rather than a reading error.
