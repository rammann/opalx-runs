...
**QUADRUPOLE**, L = 0.3 m. QUADRUPOLE K1S = +2 m^-2: skew quadrupole, x-y coupled.
...
- `Case.read_plane` reads the co-moving phase space from `quad_skew.h5`: positions in the beam frame [m], momenta in β·γ rotated into the reference direction → x′ = px/pz, y′ = py/pz, δ = |p|/p₀ − 1.
