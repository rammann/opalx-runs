...
**QUADRUPOLE**, L = 0.3 m. QUADRUPOLE K1 = -2 m^-2: focuses an electron in x.
...
With √|k| = 1.4142 m⁻¹ and L = 0.3 m the phase advance is √|k|·L = 0.4243 rad. The focusing plane (**y**) is
...
and the defocusing plane (**x**) is the same with cos→cosh, sin→sinh and the R21 sign flipped:
...
- `Case.read_plane` reads the co-moving phase space from `quad_foc_x.h5`: positions in the beam frame [m], momenta in β·γ rotated into the reference direction → x′ = px/pz, y′ = py/pz, δ = |p|/p₀ − 1.
