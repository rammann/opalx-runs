...
**QUADRUPOLE**, L = 0.3 m. QUADRUPOLE K1 = +8 m^-2: same, at a 4x stronger gradient.
...
This beam is an **electron** (`CHARGE = −1`), so a positive `K1` **defocuses** in x and focuses in y, opposite to the MAD-X convention for a positive charge. Here k = -1·K1 = -8 m⁻².

With √|k| = 2.8284 m⁻¹ and L = 0.3 m the phase advance is √|k|·L = 0.8485 rad. The focusing plane (**x**) is

```
[  cos(√k L)      sin(√k L)/√k ]   [ +0.661088  +0.265274 ]
[ -√k sin(√k L)   cos(√k L)    ] = [ -2.122192  +0.661088 ]
```

and the defocusing plane (**y**) is the same with cos→cosh, sin→sinh and the R21 sign flipped:

```
[  cosh(√k L)     sinh(√k L)/√k ]   [ +1.382125  +0.337318 ]
[  √k sinh(√k L)  cosh(√k L)    ] = [ +2.698548  +1.382125 ]
```
...
- Lattice: `DRIFT(L=0.5) → QUADRUPOLE(L=0.3) → DRIFT(L=0.5)`. Faces at s = 0.500 .. 0.800 m. `QUADRUPOLE` has **no fringe** — the field is exactly zero outside the faces.
...
- `Case.read_plane` reads the co-moving phase space from `quad_strong.h5`: positions in the beam frame [m], momenta in β·γ rotated into the reference direction → x′ = px/pz, y′ = py/pz, δ = |p|/p₀ − 1.
...
- **Tests fed:** 1 (energy), 2 (transfer matrix), 3 (symplecticity).
