...
The same map with ZREVERSE = TRUE, which turns the magnet round.
...
- Multiplier on the tabulated field: 1.0  (ZREVERSE = TRUE).
...
Must reproduce cyl_ramp_mirror to round-off, and must differ from cyl_ramp -- otherwise the profile was symmetric and the test would prove nothing.
