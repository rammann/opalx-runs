...
| `paths.py` | where things are: the repo, `studies/`, `output/`, `output_dir()`, `G4BL_FILES`, `G4BL_APP`, and `opalx()`: `$OPALX`, else the workspace build `../build/src/opalx`; `python -m opalxruns.paths --opalx` prints it. `python -m opalxruns.paths <folder>` prints a folder's place in `output/` |
